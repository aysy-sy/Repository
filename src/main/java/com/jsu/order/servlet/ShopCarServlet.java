...
public class ShopCarServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //获取前台提交的产品编号 。根据 编号搜索产品  保存到购物车中
        //同时获取用户 数量
        String num=request.getParameter("id");
         String shoppingNum=request.getParameter("shopNum");
        int id=Integer.parseInt(num);//产品编号
         int shopNum=Integer.parseInt(shoppingNum);//购物车添加数量
        ...
        try {
            ...
            flag=thingDao.saveProductToSC(product, applicantAccount,shopNum);
            if(flag==true){
